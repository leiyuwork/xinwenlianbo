...
def href(date):
    ...
    response = requests.get('http://news.cctv.com/program/xwlb/' + str(date) + '.shtml', headers=headers)
...
def news(url):
    ...
    bs_obj = BeautifulSoup(response.content.decode('gb18030'), 'lxml')
...
def save_text(date):
    f = open(r"C:\Users\ray\OneDrive\Research\PHD\Research\data\context analysis\xinwenlianbo20090626-20100505\\" + str(date) + '.txt', 'a', encoding='utf-8')
...
for date in datelist('20090626', '20100505'):
    try:
        save_text(date)
    except:
